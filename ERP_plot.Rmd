...
N170_Plot_205 <- {
  df.binerp.N170 %>% 
    ...
    ggplot(aes(y = amplitude, x = TimePoint, color = Duration, linetype = Category)) + # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_line(size = .9) +  # plot the lines
    scale_linetype_manual(values=c("solid", "dashed")) +
    scale_color_manual(values=duration_color) +
    facet_wrap(~ Hemisphere + Type, nrow = 1, labeller = label_wrap_gen(multi_line=FALSE)) +  # show the left and right hemisphere separately
...
con_Plot_205 <- {
  df.con.clean %>% 
    mutate(DuraConf = if_else(DuraConf=="17_guess", "33ms_guess",
                              if_else(DuraConf=="17_low", "33ms_low",
                                      if_else(DuraConf=="17_high", "33ms_high", "216ms_high"))),
           DuraConf = factor(DuraConf, levels = c("33ms_guess", "33ms_low", "33ms_high", "216ms_high"))) %>% 
    ggplot(aes(y = amplitude, x = TimePoint, color = DuraConf, linetype = Category)) + # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_line(size = .9) +  # plot the lines
    scale_linetype_manual(values=c("solid", "dashed")) +
    scale_color_manual(values=duration_color) +
    facet_wrap(~ Hemisphere + Category, nrow = 1, labeller = label_wrap_gen(multi_line=FALSE)) +  # show the left and right hemisphere separately
...
block_Plot_205 <- {
  df.block.clean %>% 
    mutate(DuraBlock = fct_recode(DuraBlock, `33ms_half1` = "17-1", `33ms_half2` = "17-2", `216ms_half2` = "200-2")) %>% 
    ggplot(aes(y = amplitude, x = TimePoint, color = DuraBlock, linetype = Category)) + # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_line(size = .9) +  # plot the lines
    scale_linetype_manual(values=c("solid", "dashed")) +
    scale_color_manual(values=duration_color) +
    facet_wrap(~ Hemisphere + Category, nrow = 1, labeller = label_wrap_gen(multi_line=FALSE)) +  # show the left and right hemisphere separately
