...
* **Independent variables**:
    + Stimulus Type (`NS`: *intact* vs. *scrambled*)
    + Stimulus Category (`FH`: *faces* vs. *houses*)
...
ylimit.p1 <- c(3.5, -0.5)
ylimit.n170 <- c(2, -5.5)  # y axis for plotting N170 amplitude
...
clean.beha.E1 <- {
  raw.beha.E1 %>%
    ...
    mutate(
      ...
      Duration = as_factor(Duration), # save Duration as factor
...
```{r participant information E204}
subj.info.E1 <- {
  clean.beha.E1 %>% 
...
```{r bd coding for acc E1}
# set the bd coding manually 
clean.beha.E1 <- sdif_coding_P203(clean.beha.E1)
...
```{r rainclound plot of the RT data E204, warning = FALSE}
...
```{r}
anova(lmm.rdc.P1.E1, lmm.zcp.P1.E1, refit = FALSE)
```

#### The extended model

```{r}
file.etd.P1.E1 <- file.path(folder.nesi, "E204_P1_lmm_etd.RData")

# fit the etd model
if (!file.exists(file.etd.P1.E1)) {
  lmm.etd.P1.E1 <- update(lmm.zcp.P1.E1,
                          formula = MeanAmp ~ Type * Category * Duration * Hemisphere * Response + 
                            (1 + Type_C + Cate_C + Dura_C + Hemi_C +  
                               Type_Cate + Type_Dura + Type_Hemi + Dura_Hemi +
                               Type_Cate_Dura +
...
print(summary(lmm.etd.P1.E1), corr = FALSE)
```


```{r}
summary(rePCA(lmm.etd.P1.E1))
```



```{r}
file.etd1.P1.E1 <- file.path(folder.nesi, "E204_P1_lmm_etd1.RData")

# fit the etd1 model
if (!file.exists(file.etd1.P1.E1)) {
  ...
  load(file.etd1.P1.E1)
}

print(summary(lmm.etd1.P1.E1), corr = FALSE)
``` 

```{r}
summary(rePCA(lmm.etd1.P1.E1))
```

```{r}
file.etd2.P1.E1 <- file.path(folder.nesi, "E204_P1_lmm_etd2.RData")

# fit the etd2 model
if (!file.exists(file.etd2.P1.E1)) {
  lmm.etd2.P1.E1 <- update(lmm.etd1.P1.E1,
                           formula = MeanAmp ~ Type * Category * Duration * Hemisphere * Response + 
                             (1 + Hemi_C +  
                                Type_Dura + 
                                Type_Resp + 
  ...
  load(file.etd2.P1.E1)
}

print(summary(lmm.etd2.P1.E1), corr = FALSE)
``` 

```{r}
summary(rePCA(lmm.etd2.P1.E1))
```



```{r}
file.etd3.P1.E1 <- file.path(folder.nesi, "E204_P1_lmm_etd3.RData")

# fit the etd3 model
if (!file.exists(file.etd3.P1.E1)) {
  ...
  load(file.etd3.P1.E1)
}

print(summary(lmm.etd3.P1.E1), corr = FALSE)
``` 


```{r}
anova(lmm.rdc.P1.E1, lmm.etd1.P1.E1, refit = FALSE)
```

#### The optimal model
```{r}

lmm.opt.P1.E1 <- lmm.rdc.P1.E1

anova(lmm.opt.P1.E1)
# summary(lmm.opt.P1.E1, corr = FALSE)
```

#### Diagnostic plot
```{r}
qqplot_lmer(lmm.opt.P1.E1)
```

#### Estimated marginal means
```{r}
emm.P1.E1 <- emmeans(lmm.opt.P1.E1, ~ Category + Response | Type + Duration + Hemisphere)

emm.P1.E1
```

```{r}
emmip(emm.P1.E1, Category ~ Duration | Type + Hemisphere, CIs = TRUE)
```


```{r}
pairs(emm.P1.E1, simple = "Category")
```
...
df.erp.N170.E1.long <- {
  df.erp.N170.E1 %>% 
    group_by(SubjCode, Type, Category, Duration, Hemisphere) %>% 
    summarize(mean = mean(MeanAmp), count = n()) %>% 
    filter(count > 20) %>% 
    ungroup()
}

E1.N170.anova <- aov_4(mean ~ Type * Category * Hemisphere * Duration  + (Type * Category * Hemisphere * Duration | SubjCode), df.erp.N170.E1.long)


em <- emmeans(E1.N170.anova, ~ Type * Category * Hemisphere * Duration)

emmip(em, Category ~ Duration | Type + Hemisphere, CIs = TRUE) + 
  scale_y_continuous(trans = "reverse")

pairs(em, simple = "Duration")

```


```{r}
df.N170.E1.long.resp <- {
  df.erp.N170.E1 %>% 
    filter(Type == "intact") %>% 
    group_by(SubjCode, Category, Duration, Hemisphere, Response) %>% 
    summarize(mean = mean(MeanAmp), count = n()) %>% 
    filter(count > 20) %>% 
    mutate(DuraResp = paste(Duration, Response, sep = "_")) %>% 
    filter(DuraResp != "200_0") %>% 
    ungroup()
}

df.N170.E1.subj.resp <- {
  df.N170.E1.long.resp %>%
    mutate(conditions = paste(Category, Hemisphere, DuraResp, sep = "-")) %>%
    select(SubjCode, conditions, mean) %>%
    spread(conditions, mean)  %>% 
    filter(!is.na(`face-Left-17_0`), !is.na(`house-Left-17_0`)) %$%
    SubjCode
}

df.N170.E1.long.resp %<>% filter(SubjCode %in% df.N170.E1.subj.resp)

E1.N170.anova.resp <- aov_4(mean ~ Category * Hemisphere * DuraResp + (Category * Hemisphere * DuraResp | SubjCode), df.N170.E1.long.resp)


em.resp <- emmeans(E1.N170.anova.resp, ~ Category * Hemisphere * DuraResp)

emmip(em.resp, Category ~ DuraResp | Hemisphere, CIs = TRUE) + 
  scale_y_continuous(trans = "reverse")

pairs(em.resp, simple = "DuraResp")

```


#### The maximal model
As the maximal model is too complex and cannot finish within 7 days, we will run the zcp model directly.
...
```{r}
anova(lmm.rdc.N170.E1, lmm.zcp.N170.E1, refit = FALSE)
```

#### The extended model
```{r etd lmm for E1 N170}

# file.etd.N170.E1 <- file.path(folder.nesi, "E204_N170_lmm_etd.RData")
# 
# # fit the etd model
# if (!file.exists(file.etd.N170.E1)) {
# 
...
# 
# } else {
#   load(file.etd.N170.E1)
# }
# 
# print(summary(lmm.etd.N170.E1), corr = FALSE)
...
#### The optimal model
```{r}
lmm.opt.N170.E1 <- lmm.rdc.N170.E1

# anova(lmm.opt.N170.E1)
print(summary(lmm.rdc.N170.E1), corr = FALSE)
```

```{r}
emm.N170.E1 <- emmeans(lmm.opt.N170.E1, ~ Category  | Type + Duration + Hemisphere)
```


```{r}
emmip(emm.N170.E1, Category ~ Response + Duration | Type + Hemisphere, CIs = TRUE) + 
  scale_y_continuous(trans = "reverse")
```

```{r}
pairs(emm.N170.E1, simple = "Duration")
# pairs(emm.N170.E1, simple = "Duration")
```
...
knitr::kable(desc.resp.205, digits = 4)

rt.RainPlot.205 <- {
  ggplot(data = avg.resp.long.205, aes(y = RespRate, x = as.factor(Resp), fill = Category)) + 
    geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .7) +
    ...
    geom_point(data = desc.resp.205, aes(y = Mean, x = Resp, color = Category), position = position_nudge(x = 0.3), size = 2.5) +
    geom_errorbar(data = desc.resp.205, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
...
acc.ColuPlot.205 <- {
  ggplot(data = desc.resp.205, aes(y = Mean, x = Resp, fill = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    facet_grid(Type ~ Duration) +
    geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5,
...
ratio.ColuPlot.205 <- {
  ggplot(data = desc.ratio.205, aes(y = Mean, x = Resp)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    coord_cartesian(ylim = c(0,1.1)) +  # set the limit for y axis
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "Subjective Responses for E205", x = "Responses", y = "Response Rate") +  # set the names for main, x and y axises
    scale_fill_grey() +  # set the color for columns
    # geom_text(label = c("***", "***", "", ""), color = c("red", "red", "red", "red"), size = 6, nudge_y = 0.05, nudge_x = rep(c(-0.225, 0.225), 2)) + # add starts to the significant columns
    plot_theme
}
...
p1.all.LinePlot = {
  ggplot(data = data.frame(emm.P1.E205), aes(y = emmean, x = Duration, color = Category, group = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "", x = "Hemisphere and Duration", y = expression(paste("Amplitudes ( ", mu, "V)")), fill = "Category") +  # set the names for main, x and y axises
...
##### Contrasts
```{r}
contra.all.P1 <- contrast(emmeans(lmm.opt.P1.E2, ~ Duration + Type), "pairwise", simple = "each", combine = TRUE)
...
n170.all.LinePlot = {
  ggplot(data = data.frame(emm.N170.E205), aes(y = emmean, x = Duration, color = Category, group = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    facet_grid(Type ~ Hemisphere,  switch = "x",
               labeller = labeller(Hemisphere = hemiLabel)) +
    scale_y_reverse(limits = ylimit.n170) +  # set the limit for y axis
    # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "", x = "Hemisphere and Duration", y = expression(paste("Amplitudes ( ", mu, "V)")), fill = "Category") +  # set the names for main, x and y axises
    # scale_x_discrete(labels = c("left", "right")) +
    # geom_text(label = c("", "", "", "", "***", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
...
##### Contrasts
```{r}
contra.all.N170 <- contrast(emm.N170.E205, simple = c("Category"), by = c("Duration", "Hemisphere"), adjust = "none")
...
```{r}
contrast(emmeans(lmm.opt.N170.E2, ~ Category + Duration | Type + Hemisphere), interaction = TRUE)
...
lmm.opt.P1.SC.E2 <- lmm.etd.P1.SC.E2

# print(summary(lmm.opt.P1.SC.E2), corr = FALSE)
anova(lmm.opt.P1.SC.E2)
...
##### Diagnostic plots
```{r}
qqplot_lmer(lmm.opt.P1.SC.E2)
```


##### Estimated marginal means
```{r}
emm.P1.SC.E2 <- emmeans(lmm.opt.P1.SC.E2, ~ DuraConf | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.P1.SC.E2, Category ~ DuraConf | Hemisphere, CIs = TRUE)
```

##### Contrast
```{r}
pairs(emmeans(lmm.opt.P1.SC.E2, ~ DuraConf | Category + Hemisphere))
```
...
lmm.opt.N170.SC.E2 <- lmm.etd.N170.SC.E2

print(summary(lmm.opt.N170.SC.E2), corr = FALSE)
anova(lmm.opt.N170.SC.E2)

```

##### Diagnostic plots
```{r}
qqplot_lmer(lmm.opt.N170.SC.E2)
```

##### Estimated marginal means
```{r}
emm.N170.SC.E2 <- emmeans(lmm.opt.N170.SC.E2, ~ Category + DuraConf | Hemisphere)
...
##### Plots
```{r}
emmip(emm.N170.SC.E2, Category ~ DuraConf | Hemisphere, CIs = TRUE) + 
  scale_y_continuous(trans = "reverse")
```
...
##### Contrast
```{r}
# pairs(emm.N170.SC.E2, simple = "DuraConf", by = c("Category", "Hemisphere"))
pairs(emmeans(lmm.opt.N170.SC.E2, ~ DuraConf | Category + Hemisphere), adjust = "none")
```
...
knitr::kable(desc.resp.E2.Block, digits = 4)

rt.RainPlot.E2.Block <- {
  ggplot(data = avg.resp.long.E2.Block, aes(y = RespRate, x = as.factor(Resp), fill = Category)) + 
    geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .7) +
    geom_point(aes(y = RespRate, color = Category), position = position_jitter(width = .15), size = .5, alpha = .8) +
    geom_boxplot(aes(y = RespRate), width = .2, outlier.shape = NA, alpha = .7) +
    geom_point(data = desc.resp.E2.Block, aes(y = Mean, x = Resp, color = Category), position = position_nudge(x = 0.3), size = 2.5) +
    geom_errorbar(data = desc.resp.E2.Block, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
...
acc.ColuPlot.E2.Block <- {
  ggplot(data = desc.resp.E2.Block, aes(y = Mean, x = Resp, fill = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    facet_grid(Duration ~ blockID) +
    geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5,
...
ratio.ColuPlot.E2.Block <- {
  ggplot(data = desc.ratio.E2.Block, aes(y = Mean, x = Resp)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    facet_grid(Duration ~ blockID) +
    geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5,
...
```{r}
lmm.opt.P1.high.E2 <- lmm.etd.P1.high.E2

summary(lmm.opt.P1.high.E2)
```

##### Diagnostic plots
```{r}
qqplot_lmer(lmm.opt.P1.high.E2)
```
...
```{r}
emm.P1.high.E2 <- emmeans(lmm.opt.P1.high.E2, ~ DuraBlock | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.P1.high.E2, Category ~ DuraBlock | Hemisphere, CIs = TRUE)
```

##### Contrast
```{r}
pairs(emmeans(lmm.opt.P1.high.E2, ~ DuraBlock | Category + Hemisphere))
```
...
##### The optimal model
```{r}
lmm.opt.N170.high.E2 <- lmm.etd.N170.high.E2

summary(lmm.opt.N170.high.E2)
```

##### Diagnostic plots
```{r}
qqplot_lmer(lmm.opt.N170.high.E2)
```


##### Estimated marginal means
```{r}
emm.N170.high.E2 <- emmeans(lmm.opt.N170.high.E2, ~ DuraBlock | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.N170.high.E2, Category ~ DuraBlock | Hemisphere, CIs = TRUE) +
  scale_y_continuous(trans = "reverse")
...
##### Contrast
```{r}
pairs(emmeans(lmm.opt.N170.high.E2, ~ DuraBlock | Category + Hemisphere), adjust = "none")
```




### Linear mixed model with guessing 

```{r}
df.erp.guess.E2 <- {
  df.erp.E2 %>% 
    filter(substr(Response, 2, 2) == 3) %>%
...
if (saveCSV) {
  output.erp.P1.guess.E2 <- file.path(folder.nesi, "E205_erp_P1_guess.RData")
  save(df.erp.P1.guess.E2, file = output.erp.P1.guess.E2)
}

# the structure of this dataset
head(df.erp.P1.guess.E2, 10)
```


##### The maximal model
```{r maximal lmm for P1 E2 guess}

lmm.max.P1.guess.E2 <- lmer(MeanAmp ~ Type * Category * Hemisphere * Duration +
                             (1 + Cate_C + Hemi_C + Cate_Hemi | SubjCode) +
                             (1 + Hemi_C | Stimuli),
                           data = df.erp.P1.guess.E2,
                           verbose = TRUE,
                           control = lmerControl(optimizer = "bobyqa",  # nloptwrap Nelder_Mead
                                                 optCtrl = list(maxfun = 1e5)))

print(summary(lmm.max.P1.guess.E2), corr = FALSE)

```

##### The zero-correlation-parameter model
```{r zcp lmm for P1 E2 guess}

lmm.zcp.P1.guess.E2 <- update(lmm.max.P1.guess.E2,
                             formula = MeanAmp ~ Category * Hemisphere * DuraBlock +
                               (1 + Cate_C + Hemi_C + Cate_Hemi || SubjCode) +
                               (1 + Hemi_C || Stimuli))
  

print(summary(lmm.zcp.P1.guess.E2), corr = FALSE)
...
##### The reduced model
```{r}
summary(rePCA(lmm.zcp.P1.guess.E2))
```


```{r}
lmm.rdc.P1.guess.E2 <- update(lmm.zcp.P1.guess.E2,
                             formula = MeanAmp ~ Category * Hemisphere * DuraBlock +
                             (1 + Cate_C + Hemi_C || SubjCode) +
                             (1 | Stimuli))

print(summary(lmm.rdc.P1.guess.E2), corr = FALSE)
```

```{r}
anova(lmm.rdc.P1.guess.E2, lmm.zcp.P1.guess.E2, refit = FALSE)
```

##### The extended model

```{r}
lmm.etd.P1.guess.E2 <- update(lmm.rdc.P1.guess.E2,
                             formula = MeanAmp ~ Category * Hemisphere * DuraBlock +
                             (1 + Cate_C + Hemi_C | SubjCode) +
                             (1 | Stimuli))

print(summary(lmm.etd.P1.guess.E2), corr = FALSE)
```

```{r}
anova(lmm.etd.P1.guess.E2, lmm.rdc.P1.guess.E2, refit = FALSE)
```

##### The optimal model

```{r}
lmm.opt.P1.guess.E2 <- lmm.etd.P1.guess.E2

summary(lmm.opt.P1.guess.E2)
```

##### Diagnostic plots
```{r}
qqplot_lmer(lmm.opt.P1.guess.E2)
```
...
```{r}
emm.P1.guess.E2 <- emmeans(lmm.opt.P1.guess.E2, ~ DuraBlock | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.P1.guess.E2, Category ~ DuraBlock | Hemisphere, CIs = TRUE)
```

##### Contrast
```{r}
pairs(emmeans(lmm.opt.P1.guess.E2, ~ DuraBlock | Category + Hemisphere))
```



#### Amplitudes of the N170
```{r erp df for the N170 E2 guess}
# only keep the data for amplitudes of the N170 (correct and incorrect erps)
df.erp.N170.guess.E2 <- {
  df.erp.guess.E2 %>% 
    filter(Component == "N170")
}

if (saveCSV) {
  output.erp.N170.guess.E2 <- file.path(folder.nesi, "E205_erp_N170_guess.RData")
  save(df.erp.N170.guess.E2, file = output.erp.N170.guess.E2)
}

# the structure of this dataset
head(df.erp.N170.guess.E2, 10)
```


##### The maximal model
```{r maximal lmm for N170 E2 guess}

lmm.max.N170.guess.E2 <- lmer(MeanAmp ~ Category * Hemisphere * DuraBlock +
                               (1 + Cate_C + Hemi_C + Cate_Hemi | SubjCode) +
                               (1 + Hemi_C | Stimuli),
                             data = df.erp.N170.guess.E2,
                             verbose = TRUE,
                             control = lmerControl(optimizer = "bobyqa",  # nloptwrap Nelder_Mead
                                                   optCtrl = list(maxfun = 1e5)))

print(summary(lmm.max.N170.guess.E2), corr = FALSE)

```

##### The zero-correlation-parameter model
```{r zcp lmm for N170 E2 guess}

lmm.zcp.N170.guess.E2 <- update(lmm.max.N170.guess.E2,
                               formula = MeanAmp ~ Category * Hemisphere * DuraBlock +
                                 (1 + Cate_C + Hemi_C + Cate_Hemi || SubjCode) +
                             (1 + Hemi_C || Stimuli))
  
print(summary(lmm.zcp.N170.guess.E2), corr = FALSE)
...
##### The reduced model
```{r}
summary(rePCA(lmm.zcp.N170.guess.E2))
```

```{r}
lmm.rdc.N170.guess.E2 <- update(lmm.zcp.N170.guess.E2,
                             formula = MeanAmp ~ Category * Hemisphere * DuraBlock +
                             (1 + Cate_C + Hemi_C + Cate_Hemi || SubjCode) +
                             (1 | Stimuli))

print(summary(lmm.rdc.N170.guess.E2), corr = FALSE)
```

```{r}
anova(lmm.rdc.N170.guess.E2, lmm.zcp.N170.guess.E2, refit = FALSE)
```

##### The extended model
```{r}
lmm.etd.N170.guess.E2 <- update(lmm.rdc.N170.guess.E2,
                             formula = MeanAmp ~ Category * Hemisphere * DuraBlock +
                             (1 + Cate_C + Hemi_C + Cate_Hemi | SubjCode) +
                             (1 | Stimuli))

print(summary(lmm.etd.N170.guess.E2), corr = FALSE)
```

```{r}
anova(lmm.etd.N170.guess.E2, lmm.rdc.N170.guess.E2, refit = FALSE)
```

##### The optimal model
```{r}
lmm.opt.N170.guess.E2 <- lmm.etd.N170.guess.E2

summary(lmm.opt.N170.guess.E2)
```

##### Diagnostic plots
```{r}
qqplot_lmer(lmm.opt.N170.guess.E2)
```


##### Estimated marginal means
```{r}
emm.N170.guess.E2 <- emmeans(lmm.opt.N170.guess.E2, ~ DuraBlock | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.N170.guess.E2, Category ~ DuraBlock | Hemisphere, CIs = TRUE) +
  scale_y_continuous(trans = "reverse")
...
##### Contrast
```{r}
pairs(emmeans(lmm.opt.N170.guess.E2, ~ Category | DuraBlock + Hemisphere), adjust = "none")
```
