...
Three random effects of Stimuli were removed.
...
##### The extended model
```{r extended glmm for E204 RT}

file.etd.rt.E4 <- file.path(folder.nesi, "E204_rt_glmm_etd.RData")
...
print(summary(glmm.etd.rt.E4), corr = FALSE)
```
...
```{r}
summary(rePCA(glmm.etd.rt.E4))
```


```{r extended2 glmm for E204 RT}

file.etd2.rt.E4 <- file.path(folder.nesi, "E204_rt_glmm_etd2.RData")

# fit the reduced model
if (!file.exists(file.etd2.rt.E4)) {
  ...
  load(file.etd2.rt.E4)
}

print(summary(glmm.etd2.rt.E4), corr = FALSE)
```

```{r extended3 glmm for E204 RT}

file.etd3.rt.E4 <- file.path(folder.nesi, "E204_rt_glmm_etd3.RData")

# fit the reduced model
if (!file.exists(file.etd3.rt.E4)) {
  glmm.etd3.rt.E4 <- update(glmm.etd2.rt.E4,
                           formula = RT ~ Type * Category * Duration +
                             (1 + Type_D + Type_Cate + Type_Dura + Type_Cate_Dura | SubjCode) +
                             (0 + Type_D + Type_Dura + Type_Cate_Dura | Stimuli))
} else {
  load(file.etd3.rt.E4)
}

print(summary(glmm.etd3.rt.E4), corr = FALSE)
```
...
```{r}
summary(rePCA(lmm.etd1.rt.E4))
```

```{r extended2 lmm for E204 RT}
file.etd2.rt.E4 <- file.path(folder.nesi, "E204_rt_lmm_etd2.RData")
...
print(summary(lmm.etd2.rt.E4), corr = FALSE)

```


```{r}
anova(lmm.etd2.rt.E4, lmm.rdc.rt.E4)
```
