...
clean.beha.E1 <- sdiff_coding_P203(clean.beha.E1)
...
ratio.ColuPlot.E2 <- {
  ggplot(data = desc.ratio.E2, aes(y = Mean, x = Resp)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    facet_grid(Type ~ Duration) +
    geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5,
...
##### Diagnostic plots
```{r qqplot for lmm.opt.P1.E2}
# qqplot
qqplot_lmer(lmm.opt.P1.E2)
```

##### Estimated marginal means
```{r}
emm.P1.E2 <- emmeans(lmm.opt.P1.E2, ~ Type + Duration | Hemisphere + Category )
...
##### Plots
```{r}
emmip(emm.P1.E2, Duration ~ Type | Hemisphere + Category, CIs = TRUE)

```

##### Contrast
```{r}
# contrast(emm.P1.E2, simple = "each", combine = TRUE)
contrast(emm.P1.E2)

# contrast(emmeans(lmm.opt.P1.E2, ~ Type + Duration))
```

```{r}
contrast(emm.P1.E2, interaction = TRUE)

# contrast(emmeans(lmm.opt.P1.E2, ~ Type + Duration | Hemisphere + Category), interaction = TRUE)
...
lmm.opt.N170.E2 <- lmm.rdc.N170.E2

print(summary(lmm.opt.N170.E2), corr = FALSE)
anova(lmm.opt.N170.E2)
...
# qqplot
qqplot_lmer(lmm.opt.N170.E2)
```

##### Estimated marginal means
```{r}
emm.N170.E2 <- emmeans(lmm.opt.N170.E2,  ~ Duration + Category | Hemisphere + Type )
...
##### Plots
```{r}
emmip(emm.N170.E2, Category ~ Duration | Type + Hemisphere, CIs = TRUE) + 
...
##### Contrast
```{r}
# contrast(emm.N170.E2, simple = "each", combine = TRUE)
pairs(emm.N170.E2, simple = c("Duration"), by = c("Category", "Hemisphere"), adjust = "none")
...
##### Contrast
```{r}
pairs(emmeans(lmm.opt.N170.high.E2, ~ DuraBlock | Category + Hemisphere), adjust = )
```



### Linear mixed model with subjective confidences for 17ms
```{r}
df.erp.17.E2 <- {
  df.erp.E2 %>% 
    filter(Duration == "17", Type == "intact") %>% 
    mutate(Confidence = if_else(Response %in% c("11", "55"), "high", 
                                if_else(Response %in% c("12", "54"), "low",
                                        if_else(substring(Response, 2) == "3", "guess", "NA")))) %>% 
    filter(Confidence != "NA") %>% 
    sdif_coding_E205_erp_17()
}
...
if (saveCSV) {
  output.erp.P1.17.E2 <- file.path(folder.nesi, "E205_erp_P1_17.RData")
  save(df.erp.P1.17.E2, file = output.erp.P1.17.E2)
}

# the structure of this dataset
head(df.erp.P1.17.E2, 10)
```


##### The maximal model
```{r maximal lmm for P1 E2 17}
file.max.P1.17.E2 <- file.path(folder.nesi, "E205_P1_17_lmm_max.RData")

# fit the maximal model
if (!file.exists(file.max.P1.17.E2)) {
  ...
  load(file.max.P1.17.E2)
}

print(summary(lmm.max.P1.17.E2), corr = FALSE)

```

##### The zero-correlation-parameter model
```{r zcp lmm for P1 E2 17}
file.zcp.P1.17.E2 <- file.path(folder.nesi, "E205_P1_17_lmm_zcp.RData")

# fit the zcp model
if (!file.exists(file.zcp.P1.17.E2)) {
  ...
  load(file.zcp.P1.17.E2)
}

print(summary(lmm.zcp.P1.17.E2), corr = FALSE)

```

```{r}
summary(rePCA(lmm.zcp.P1.17.E2))
```

```{r rdc lmm for P1 E2 17}
file.rdc.P1.17.E2 <- file.path(folder.nesi, "E205_P1_17_lmm_rdc.RData")

# fit the rdc model
if (!file.exists(file.rdc.P1.17.E2)) {
  lmm.rdc.P1.17.E2 <- update(lmm.zcp.P1.17.E2,
                             ...
                             (1 + ConLH_C + ConGL_C || Stimuli))
  
} else {
  load(file.rdc.P1.17.E2)
}

print(summary(lmm.rdc.P1.17.E2), corr = FALSE)

```


```{r}
anova(lmm.rdc.P1.17.E2, lmm.zcp.P1.17.E2, refit = FALSE)
```



```{r etd lmm for P1 E2 17}
file.etd.P1.17.E2 <- file.path(folder.nesi, "E205_P1_17_lmm_etd.RData")

# fit the etd model
if (!file.exists(file.etd.P1.17.E2)) {
  lmm.etd.P1.17.E2 <- update(lmm.zcp.P1.17.E2,
                             formula = MeanAmp ~ Category * Hemisphere * Confidence +
                             (1 + Cate_C + Hemi_C + ConLH_C + ConGL_C + 
                                Cate_ConGL 
                              | SubjCode) +
  ...
  load(file.etd.P1.17.E2)
}

print(summary(lmm.etd.P1.17.E2), corr = FALSE)

```

```{r}
anova(lmm.etd.P1.17.E2, lmm.rdc.P1.17.E2, refit = FALSE)
```


```{r}
lmm.opt.P1.17.E2 <- lmm.etd.P1.17.E2
...
if (saveCSV) {
  output.erp.N170.17.E2 <- file.path(folder.nesi, "E205_erp_N170_17.RData")
  save(df.erp.N170.17.E2, file = output.erp.N170.17.E2)
}

# the structure of this dataset
head(df.erp.N170.17.E2, 10)
```


##### The maximal model
```{r maximal lmm for E2 N170 17}
file.max.N170.17.E2 <- file.path(folder.nesi, "E205_N170_17_lmm_max.RData")

# fit the maximal model
if (!file.exists(file.max.N170.17.E2)) {
  ...
  load(file.max.N170.17.E2)
}

print(summary(lmm.max.N170.17.E2), corr = FALSE)

```


##### The zero-correlation-parameter model
```{r zcp lmm for E2 N170 17}

file.zcp.N170.17.E2 <- file.path(folder.nesi, "E205_N170_17_lmm_zcp.RData")

# fit the zcp model
if (!file.exists(file.zcp.N170.17.E2)) {
  ...
  load(file.zcp.N170.17.E2)
}


print(summary(lmm.zcp.N170.17.E2), corr = FALSE)
