...
tmp <- sapply(list.files(folder_R, "*.R", full.names = TRUE, recursive = TRUE), source)
```
...
```{r}
p1_high_LinePlot_bw = {
  ggplot(data = data.frame(emm.P1.high.E205), aes(y = emmean, x = DuraBlock, group = Category)) +  # set the data, variables for x and y axises, and the variable for dividing data
    geom_point(aes(fill = DuraBlock), size=4, shape=21, stroke=0) +
