...
desc.rt.205 <- {
  avg.rt.long.205 %>%
    group_by(Type, Category, Duration, Resp) %>%
    summarize(Mean = mean(CRT)
              , N = n()
              , SD = if_else(N > 1, sd(CRT), 0)
              , SE = SD/sqrt(N)
              , SE.lo = Mean - SE, SE.hi = Mean + SE
              , CI_lo = Mean - SE * 1.96
              , CI_hi = Mean + SE * 1.96
...
```{r rainclound plot of the CRT data E205, warning = FALSE}
knitr::kable(desc.rt.205, digits = 4)

rt.RainPlot.205 <- {
  ggplot(data = avg.rt.long.205, aes(y = CRT, x = as.factor(Resp), fill = Category)) + 
    geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .7) +
    geom_point(aes(y = CRT, color = Category), position = position_jitter(width = .15), size = .5, alpha = .8) +
    geom_boxplot(aes(y = CRT), width = .2, outlier.shape = NA, alpha = .7) +
    geom_point(data = desc.rt.205, aes(y = Mean, x = Resp, color = Category), position = position_nudge(x = 0.3), size = 2.5) +
    geom_errorbar(data = desc.rt.205, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
    facet_grid(Type ~ Duration) +
...
rt.ColuPlot.205 <- {
  ggplot(data = desc.rt.205, aes(y = Mean, x = Resp, fill = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    facet_grid(Type ~ Duration, 
               labeller = labeller(Duration = dura.labels)) +
    geom_errorbar(mapping = aes(ymin = CI_lo, ymax = CI_hi), linetype = 1,  # set the error bar
                  ...
                  position = position_dodge(width=0.9)) +
    geom_hline(yintercept = c(1), linetype = 5, alpha = 0.5) +  # add the line for 0.5 and 1 (y)
    coord_cartesian(ylim = c(0,1750)) +  # set the limit for y axis
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "", x = "Responses", y = "Response times (ms)", fill = "Category") +  # set the names for main, x and y axises Subjective Responses for E205
    scale_fill_grey() +  # set the color for columns
    # geom_text(label = c("***", "***", "", ""), color = c("red", "red", "red", "red"), size = 6, nudge_y = 0.05, nudge_x = rep(c(-0.225, 0.225), 2)) + # add starts to the significant columns
    erp_theme
}
...
#### LMM for correct responses
```{r}
clean.beha.205 %<>% sdif_coding_P203 
```

##### The maximal model
```{r}

lmm.max.rt <- lmer(RT ~ Type * Category * Duration * Resp + 
                     (1 + Type_C + Cate_C + Dura_C +
                        Type_Cate + Type_Dura + Cate_Dura + 
                        Type_Cate_Dura 
                      | SubjCode), 
                   clean.beha.205)
...
Category was removed from the fixed effects due to the rank deficient warning. Also, no significant differences of any effects on Category were observed in the summary of previous model. 

```{r}

lmm.max.rt <- lmer(RT ~ Type * Duration * Resp + Category +
                     (1 + Type_C + Cate_C + Dura_C +
                        Type_Cate + Type_Dura + Cate_Dura + 
                        Type_Cate_Dura 
                      | SubjCode), 
...
lmm.zcp.rt <- lmer(RT ~ Type * Duration * Resp + Category +
                     (1 + Type_C + Cate_C + Dura_C +
                        Type_Cate + Type_Dura + Cate_Dura + 
                        Type_Cate_Dura 
                      || SubjCode), 
...
##### The reduced model
```{r}
summary(rePCA(lmm.zcp.rt))


lmm.rdc.rt <- update(lmm.zcp.rt,
                     formula = RT ~ Type * Duration * Resp + Category +
                       (1 + Type_C + Cate_C + Dura_C +
                          Type_Cate + Type_Dura + Cate_Dura + 
                          Type_Cate_Dura 
                        || SubjCode))
...
# summary(lmm.rdc.)
anova(lmm.rdc.rt, lmm.zcp.rt, refit = FALSE)
...
```{r}
anova(lmm.etd.rt, lmm.rdc.rt, refit = FALSE)
```

##### The optimal model
```{r}
lmm.opt.rt <- lmm.etd.rt 
...
#### Estimated marginal means
```{r}
emm.rt <- emmeans(lmm.opt.rt, ~ Resp + Duration + Type)

as.data.frame(emm.rt) %>% 
  select(Type, Duration, Resp , everything())
```

#### Contrasts
```{r}
contrast(emm.rt, "pairwise", simple = "each", combine = TRUE)
```
...
p1.all.LinePlot = {
  ggplot(data = data.frame(emm.P1.E205), aes(y = emmean, x = Duration, color = Category, group = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "", x = "Duration (ms)", y = expression(paste("Amplitudes ( ", mu, "V)")), fill = "Category") +  # set the names for main, x and y axises
...
```{r}
contrast(emmeans(lmm.opt.N170.E205, ~ Category + Duration | Type + Hemisphere), interaction = TRUE)
...
##### Estimated marginal means
```{r}
emm.P1.SC.E205 <- emmeans(lmm.opt.P1.SC.E2, ~ DuraConf | Category + Hemisphere)
...
p1.conf.LinePlot = {
  ggplot(data = data.frame(emm.P1.SC.E205), aes(y = emmean, x = DuraConf, color = Category, group = Category)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "", x = "Duration and Subjective Confidence", y = expression(paste("Amplitudes ( ", mu, "V)")), fill = "Category") +  # set the names for main, x and y axises
    # scale_x_discrete(labels = c("left", "right")) +
    # geom_text(label = c("", "", "", "", "***", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
    theme_bw() +
    erp_theme
...
##### Contrast
```{r}
pairs(emmeans(lmm.opt.P1.SC.E205, ~ DuraConf | Category + Hemisphere))
```
...
##### Estimated marginal means
```{r}
emm.N170.SC.E205 <- emmeans(lmm.opt.N170.SC.E2, ~ Category + DuraConf | Hemisphere)
...
pairs(emmeans(lmm.opt.N170.SC.E205, ~ DuraConf | Category + Hemisphere), adjust = "none")
```
...
##### Contrast
```{r}
pairs(emmeans(lmm.opt.P1.high.E205, ~ DuraBlock | Category + Hemisphere))
```
...
##### Contrast
```{r}
pairs(emmeans(lmm.opt.N170.high.E205, ~ DuraBlock | Category + Hemisphere), adjust = "none")
```
...
df.erp.guess.E2 <- {
  df.erp.E2 %>% 
    mutate(DuraType = paste(Duration, Type, sep = "-")) %>% 
    filter(substr(Response, 2, 2) == 3,
           !(DuraType == "200-intact")) %>%
    mutate(DuraType = as.factor(DuraType)) %>% 
    sdif_coding_P203_erp()
  #   
  #   sdif_coding_E205_erp_conf()
}

contrasts(df.erp.guess.E2$DuraType) <- MASS::contr.sdif(nlevels(df.erp.guess.E2$DuraType)) 

df.erp.guess.E2 %<>%
  mutate(DT_SI = if_else(DuraType == "17-intact", -0.6666667, 0.3333333),
         DT_20017 = if_else(DuraType == "200-scrambled", 0.6666667, -0.3333333),
         
         Cate_SI = Cate_C * DT_SI,
         Cate_20017 = Cate_C * DT_20017,
         
         Hemi_SI = Hemi_C * DT_SI,
         Hemi_20017 = Hemi_C * DT_20017,
         
         Cate_Hemi_SI = Cate_Hemi * DT_SI,
         Cate_Hemi_20017 = Cate_Hemi * DT_20017)
...
# lmm.max.P1.guess.E2 <- lmer(MeanAmp ~ Category * Hemisphere * DuraType +
#                               (1 + Cate_C + Hemi_C + Cate_Hemi +
#                                  DT_SI + DT_20017 + 
#                                  Cate_SI + Cate_20017 + Hemi_SI + Hemi_20017 +
#                                  Cate_Hemi_SI + Cate_Hemi_20017 
#                                | SubjCode) +
#                               (1 + Hemi_C +
#                                  DT_SI + DT_20017 + 
#                                  Hemi_SI + Hemi_20017
...
lmm.zcp.P1.guess.E2 <- lmer(MeanAmp ~ Category * Hemisphere * DuraType +
                              (1 + Cate_C + Hemi_C + Cate_Hemi +
                                 DT_SI + DT_20017 + 
                                 Cate_SI + Cate_20017 + Hemi_SI + Hemi_20017 +
                                 Cate_Hemi_SI + Cate_Hemi_20017 
...
lmm.rdc.P1.guess.E2 <- update(lmm.zcp.P1.guess.E2,
                              formula = MeanAmp ~ Category * Hemisphere * DuraType +
                                (1 + Cate_C + Hemi_C + 
                                   DT_SI + DT_20017 + 
                                   Cate_SI + Cate_20017 + Hemi_20017
                                 || SubjCode) +
                                (1 + DT_SI + DT_20017
                                 || Stimuli))
...
```{r}
summary(rePCA(lmm.etd.P1.guess.E2))
```


```{r}
lmm.etd1.P1.guess.E2 <- update(lmm.etd.P1.guess.E2,
                             formula =  MeanAmp ~ Category * Hemisphere * DuraType +
                                (1 + Hemi_C + 
                                   DT_SI + DT_20017 + 
                                   Cate_SI + Hemi_20017
...
print(summary(lmm.etd1.P1.guess.E2), corr = FALSE)
```


```{r}
summary(rePCA(lmm.etd1.P1.guess.E2))
```


```{r}
lmm.etd2.P1.guess.E2 <- update(lmm.etd1.P1.guess.E2,
                             formula =  MeanAmp ~ Category * Hemisphere * DuraType +
                                (1 + Hemi_C + 
                                   DT_SI + DT_20017 + 
                                   Cate_SI 
...
print(summary(lmm.etd2.P1.guess.E2), corr = FALSE)
```


```{r}
anova(lmm.etd2.P1.guess.E2, lmm.rdc.P1.guess.E2, refit = FALSE)
```

##### The optimal model

```{r}
lmm.opt.P1.guess.E2 <- lmm.rdc.P1.guess.E2
...
```{r}
emm.P1.guess.E2 <- emmeans(lmm.opt.P1.guess.E2, ~ DuraType | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.P1.guess.E2, Category ~ DuraType | Hemisphere, CIs = TRUE)
```

##### Contrast
```{r}
contrast(emm.P1.guess.E2, "pairwise", simple = "each", combine = TRUE)
```
...
# lmm.max.N170.guess.E2 <- lmer(MeanAmp ~ Category * Hemisphere * DuraType +
#                               (1 + Cate_C + Hemi_C + Cate_Hemi +
#                                  DT_SI + DT_20017 + 
#                                  Cate_SI + Cate_20017 + Hemi_SI + Hemi_20017 +
#                                  Cate_Hemi_SI + Cate_Hemi_20017 
...
lmm.zcp.N170.guess.E2 <- lmer(MeanAmp ~ Category * Hemisphere * DuraType +
                              (1 + Cate_C + Hemi_C + Cate_Hemi +
                                 DT_SI + DT_20017 + 
                                 Cate_SI + Cate_20017 + Hemi_SI + Hemi_20017 +
                                 Cate_Hemi_SI + Cate_Hemi_20017 
...
```{r}
summary(rePCA(lmm.etd.N170.guess.E2))
```


```{r}
lmm.etd1.N170.guess.E2 <- update(lmm.etd.N170.guess.E2,
                                 formula = MeanAmp ~ Category * Hemisphere * DuraType +
                                   (1 + Hemi_C + 
                                      DT_SI + DT_20017 + 
                                      Cate_20017 + Hemi_20017  
                                    | SubjCode) +
                                   (1 + DT_SI + DT_20017
                                    | Stimuli))

print(summary(lmm.etd1.N170.guess.E2), corr = FALSE)
```



```{r}
anova(lmm.etd1.N170.guess.E2, lmm.rdc.N170.guess.E2, refit = FALSE)
```

##### The optimal model
```{r}
lmm.opt.N170.guess.E2 <- lmm.etd1.N170.guess.E2
...
##### Estimated marginal means
```{r}
emm.N170.guess.E2 <- emmeans(lmm.opt.N170.guess.E2, ~ DuraType | Category + Hemisphere)
...
##### Plots
```{r}
emmip(emm.N170.guess.E2, Category ~ DuraType | Hemisphere, CIs = TRUE) +
  scale_y_continuous(trans = "reverse")
...
##### Contrast
```{r}
pairs(emm.N170.guess.E2, adjust = "none")
```
