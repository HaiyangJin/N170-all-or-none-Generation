...
### LMM for responses
```{r}
df_ratio_205 %<>% sdif_coding_P203_resp 
```
...
contra.resp[c(1, 5)]
summary(contra.resp[c(1, 5)], delta = .05, level = .9)
```

### One-sample t-test
```{r}
summary(emm.resp[c(13, 18)], null = 0.5, delta = .1, infer = TRUE, level=.9)
summary(emm.resp[c(13, 18)], null = 0.6, side = "<", infer = TRUE)
summary(emm.resp[c(13, 18)], null = 0.4, side = ">", infer = TRUE)
```

### Logisitc mixed models for responses
```{r}
library(optimx)

df_glmm_205 <- clean_beha_205 %>%
  mutate(isFace = if_else(Category == "face", 1, 0)) %>% 
  sdif_coding_P203_resp 
```

#### The maximal model
```{r}
# glmm_max_resp <- glmer(isFace ~ Type * Duration * Resp +
#                          (1 + Type_C + Dura_C + Type_Dura | SubjCode), 
#                        family = binomial("logit"),
#                        df_glmm_205)
# 
# print(summary(glmm_max_resp), corr = FALSE)
...
glmm_zcp_resp <- glmer(isFace ~ Type * Duration * Resp +
                         (1 + Type_C + Dura_C + Type_Dura || SubjCode), 
                       family = binomial("logit"),
                       df_glmm_205)

print(summary(glmm_zcp_resp), corr = FALSE)
 
```

#### The reduced model
```{r}
summary(rePCA(glmm_zcp_resp))

glmm_rdc_resp <- update(glmm_zcp_resp,
                        formula = isFace ~ Type * Duration * Resp + 
                          (0 + Dura_C + Type_Dura || SubjCode),
                        control = glmerControl(optimizer = "optimx", # calc.derivs = FALSE,
                                               optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

print(summary(glmm_rdc_resp), corr = FALSE)
...
```{r}
summary(rePCA(glmm_rdc_resp))

glmm_rdc_resp1 <- update(glmm_zcp_resp,
                        formula = isFace ~ Type * Duration * Resp + 
                          (0 + Type_Dura || SubjCode),
                        control = glmerControl(optimizer = "optimx", # calc.derivs = FALSE,
                                               optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

print(summary(glmm_rdc_resp1), corr = FALSE)

```

```{r}
summary(rePCA(glmm_rdc_resp1))

glmm_rdc_resp2 <- update(glmm_rdc_resp1,
                        formula = isFace ~ Type * Duration * Resp + 
                          (1 | SubjCode),
                        control = glmerControl(optimizer = "optimx", # calc.derivs = FALSE,
                                               optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
